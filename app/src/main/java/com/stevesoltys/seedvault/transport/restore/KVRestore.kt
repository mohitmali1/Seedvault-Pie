...
internal class KVRestore(
    ...
    private fun getSortedKeys(token: Long, packageInfo: PackageInfo): List<DecodedKey>? {
        val records: List<String> = try {
            Log.e(TAG, "RecordsList ${plugin.listRecords(token, packageInfo)}")
